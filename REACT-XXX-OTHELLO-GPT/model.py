...
class GPT(nn.Module):
    ...
    loading pretrained weights (from Hugging Face or safetensors files), and text generation.
    ...
    def load_safetensors(self, filepath):
        ...
        for key in model_keys.intersection(file_keys):
            if model_state_dict[key].shape != state_dict[key].shape:
                print(f"Warning: Shape mismatch for {key}. Expected {model_state_dict[key].shape}, got {state_dict[key].shape}")
                continue
